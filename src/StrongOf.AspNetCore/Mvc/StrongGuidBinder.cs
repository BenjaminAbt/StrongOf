...
/// <summary>
/// Binds incoming MVC values to <see cref="StrongGuid{TStrong}"/> instances.
/// </summary>
/// <typeparam name="TStrong">Concrete StrongOf GUID type to materialize.</typeparam>
public class StrongGuidBinder<TStrong> : StrongOfBinder
    where TStrong : StrongGuid<TStrong>, IStrongOf<Guid, TStrong>
{
    /// <summary>
    /// Attempts to parse the raw request value as the configured GUID StrongOf type.
    /// </summary>
    ...
    {
        if (StrongGuid<TStrong>.TryParse(value, null, out TStrong? strong))
        {
